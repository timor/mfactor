...
- =Gemfile[.lock]= :: Ruby packages version, see ruby gems documentation
- =Rakefile= :: for mfactor development only
- =instructionset.yml= :: this file defines the primitive instructions for the [[VM Implementation][VM
     Implementation]].  Note that the host compiler gets its list of supported instructions
     from =src/mfactor/primitives.mfactor=.

*** =html=
contains generated documentation

*** =lib=
Ruby source code for the [[Host Compiler][Host Compiler]].
...
libraries (see [[Supplied Libraries]])

- =interpreter.[ch]= :: source of the [[VM Implementation][VM Implementation]]
- =reader.c= :: simple tokenizer implementation
- =seq_headers.h= :: definitions for inline data headers(e.g. used by =litc= in [[literal-instructions]]

The following headers have target-dependent implementations in =target/=
- =reset_system.h= :: toggle system/interpreter reset
- =runtime.h= :: platform-specifics for measuring run times

**** =mfactor=
Supplied MFactor libraries (see [[Supplied Libraries]]).

*** =tasks=
=mfactor.rake=: The file that defines the =mfactor= rake task that is used to integrate
the VM into the host application (see [[rake-intro]])
...
- =MFACTOR_FF=: path to file containing interface definitions to c code (see [[ffi-intro]])
...
=bytecode.c=.  Note that is not really possible to access entries in that array by index,
...
variable (see [[rake-intro]]) into an address.
