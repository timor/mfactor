...
module MFactor
  ...
  # gotos.  It should be possible to emit all backwards jumps as
  ...
  class CEmitter
    ...
    @@custom_builtins = { 
      ...
      "nop" => proc{|args| ""},
      "emit" => proc{|char| "puts(char)"}
    }
    def initialize (user_overrides)
      @user_overrides = {}
      if user_overrides
        puts "user overrides: #{user_overrides}"
...
#      @names={}           # local variable and formal parameter names in this definition
    ...
    def follow_control(node)    # returns false if node was already visited
      ...
      when MFCompiledCall then  # emit function call, follow body
        ...
        maybe_override = get_override_representation(call.definition.name.to_s, actuals.map{|i| get_representation(i)})
        if maybe_override
          s << maybe_override
        else
          raise CompileError, "no override defined for primitive: #{call.definition.name}" if call.definition.primitive? and
            !maybe_override
